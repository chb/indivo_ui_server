		...
		if ( this.lastAppId ) {
			...
			if ( appId === this.lastAppId && this.appMap[appId] ) {
				...
				// otherwise, let the app_manager handle it
					...
					APP_MANAGER.stop_managing_app(appModel.MANAGER_uuid);
				}
	...
	selectTab: function( el ) {
		...
		if ( selected.is('*') && (!el || selected.attr('id') != el.attr('id')) ) {
			var sel_clone = selected.clone(false).css({
				...
				'height': selected.innerHeight() - 8 + 'px',
				/* 8 = 4px + 4px top and bottom padding. Would be better to get this from CSS or calculate it! */
				'-moz-box-shadow': 'none',

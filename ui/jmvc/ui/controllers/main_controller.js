 ...
 *
 *
 * TODO: some of these methods would be better as class/static (e.g. _add_app called by the record controller). refactor
 *
 */
...
/* @Prototype */
{
  ...
  add_app: function(params) {
    ...
    if (pha.data.ui) {
      $('#active_app_tabs').tabs('add', '#'+pha.id.replace(/@/, '_at_').replace(/\./g,'_')  , pha.data.name);
      ...
      $('#active_app_tabs_inner li:last').prepend(line);          // add the icon
      ...
      $('#active_app_tabs_inner li:last img').click(click_handler) // a bit hackish: refactor by wrapping both in <a>?
      ...
      $('#background_app_tabs_inner li:last a').prepend(line);
